...
const DomicileContent = ({ currentUser }) => {
  ...
  const onSearch = async () => {
    ...
    try {
        const res1 = (await getListAnnonces(1,undefined,undefined,recherche.search,recherche.min, recherche.max)).content;
        const res2 = (await getListAnnonces(2,undefined,undefined,recherche.search,recherche.min, recherche.max)).content;
        setAnnonces(res1.concat(res2));
      }
     catch (error) {
      console.log(error);
